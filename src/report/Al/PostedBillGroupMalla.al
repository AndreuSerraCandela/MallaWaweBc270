/// <summary>
/// Report Post Bill Group Malla (ID 50027).
/// </summary>
report 50020 "Post Bill Group Malla"
{
        ...
        dataitem(BillGr; "Bill Group")
        {
            ...
            dataitem(Doc; "Cartera Doc.")
            {
                ...
                begin
                    ...
                    if "Entry No." <> 0 THEN
                        if NOT CustLedgEntry.GET("Entry No.") THEN BEGIN
                            ...
                            CustLedgEntry."Reason Code" := 'SIN FAC';
                            Cust.Get("Account No.");
                            CustLedgEntry."Customer Posting Group" := Cust."Customer Posting Group";
                            CustLedgEntry."Global Dimension 1 Code" := "Global Dimension 1 Code";
                            CustLedgEntry."Global Dimension 2 Code" := "Global Dimension 2 Code";
                        END;
                    ...
                    if CustLedgEntry."Document Type" <> CustLedgEntry."Document Type"::Bill Then BalanceAccount := CustPostingGr."Receivables Account";
                    if CustLedgEntry."Reason Code" = 'SIN FAC' Then BalanceAccount := CustPostingGr."Receivables Account";
                ...
                var
                    ...
                    PDoc: Record "Posted Cartera Doc.";
                begin
                    ...
                    repeat
                        if TempBGPOPostBuffer."Entry No." <> 0 THEN
                            if NOT CustLedgEntry2.GET(TempBGPOPostBuffer."Entry No.") THEN BEGIN
                                if Not Doc.Get(Doc.Type::Receivable, TempBGPOPostBuffer."Entry No.") Then BEGIN
                                    PDoc.Get(pDoc.Type::Receivable, TempBGPOPostBuffer."Entry No.");
                                    Doc."Document No." := pDoc."Document No.";
                                    Doc."Account No." := Doc."Account No.";
                                end;
                                CustLedgEntry2.INIT;
                                CustLedgEntry2."Entry No." := TempBGPOPostBuffer."Entry No.";
                                CustLedgEntry2."Customer No." := Doc."Account No.";
                                Cust.Get(Doc."Account No.");
                                CustLedgEntry."Document No." := Doc."Document No.";
                                CustLedgEntry2."Customer Posting Group" := Cust."Customer Posting Group";
                                CustLedgEntry2."Global Dimension 1 Code" := TempBGPOPostBuffer."Global Dimension 1 Code";
                                CustLedgEntry2."Global Dimension 2 Code" := TempBGPOPostBuffer."Global Dimension 2 Code";
                            END;
                        ...
                        if FeeRange.GetTotalDiscExpensesAmt <> 0 then begin
                            ...
                            for i := 0 to NoRegs - 1 do begin
                                ...
                                if NOT CustLedgEntry2.GET(TempBGPOPostBuffer."Entry No.") THEN BEGIN
                                    ...
                                    CustLedgEntry2."Entry No." := TempBGPOPostBuffer."Entry No.";
                                    CustLedgEntry2."Customer No." := Doc."Account No.";
                                    CustLedgEntry2."Reason Code" := 'SIN FAC';
                                    Cust.Get(Doc."Account No.");
                                    CustLedgEntry2."Document No." := Doc."Document No.";
                                    CustLedgEntry2."Customer Posting Group" := Cust."Customer Posting Group";
                                    CustLedgEntry2."Global Dimension 1 Code" := TempBGPOPostBuffer."Global Dimension 1 Code";
                                    CustLedgEntry2."Global Dimension 2 Code" := TempBGPOPostBuffer."Global Dimension 2 Code";
                                END;
    ...
    var
        ...
        Text1100002: Label 'Posting Documentos de cobros  #2######';
    ...
    begin
        ...
        repeat
            ...
            PostedDoc.Insert();
            if NOT CustLedgEntry.GET(PostedDoc."Entry No.") THEN BEGIN
                CustLedgEntry.INIT;
                CustLedgEntry."Entry No." := PostedDoc."Entry No.";
                CustLedgEntry."Customer No." := PostedDoc."Account No.";
                CustLedgEntry."Reason Code" := 'SIN FAC';
                CustLedgEntry."Document No." := PostedDoc."Document No.";
                Cust.Get(PostedDoc."Account No.");
                CustLedgEntry."Customer Posting Group" := Cust."Customer Posting Group";
                CustLedgEntry."Global Dimension 1 Code" := PostedDoc."Global Dimension 1 Code";
                CustLedgEntry."Global Dimension 2 Code" := PostedDoc."Global Dimension 2 Code";
            END ELSE BEGIN

                CustLedgEntry."Document Situation" := CustLedgEntry."Document Situation"::"Posted BG/PO";
                case PostedDoc.Status Of
    ...
    begin
        ...
        if CustLedgEntry."Document No." = '' Then begin
            if Not Doc.Get(Doc.Type::Receivable, TempBGPOPostBuffer."Entry No.") Then BEGIN
                PDoc.Get(pDoc.Type::Receivable, TempBGPOPostBuffer."Entry No.");
                Doc."Document No." := pDoc."Document No.";
            end;
            CustLedgEntry.INIT;
            CustLedgEntry."Entry No." := TempBGPOPostBuffer."Entry No.";
            CustLedgEntry."Customer No." := Doc."Account No.";
            CustLedgEntry."Document No." := Doc."Document No.";
            Cust.Get(Doc."Account No.");
            CustLedgEntry."Customer Posting Group" := Cust."Customer Posting Group";
            CustLedgEntry."Global Dimension 1 Code" := TempBGPOPostBuffer."Global Dimension 1 Code";
            CustLedgEntry."Global Dimension 2 Code" := TempBGPOPostBuffer."Global Dimension 2 Code";
        end;
        with GenJnlLine do begin
            Clear(GenJnlLine);
            Init;
    ...
    /// InitReqForm.
    /// </summary>
    /// <param name="TemplName2">Code[10].</param>
    /// <param name="BatchName2">Code[10].</param>
    [Scope('OnPrem')]
    ...
    /// PostDocToFactoring.
    /// </summary>
    /// <param name="Doc">Record "Cartera Doc.".</param>
    //[Scope('OnPrem')]
    ...
    begin
        with Doc do begin
            if NOT CustLedgEntry.GET(Doc."Entry No.") THEN BEGIN
                CustLedgEntry.INIT;
                if Not Doc.Get(Doc.Type::Receivable, TempBGPOPostBuffer."Entry No.") Then BEGIN
                    PDoc.Get(pDoc.Type::Receivable, TempBGPOPostBuffer."Entry No.");
                    Doc."Document No." := pDoc."Document No.";
                end;
                CustLedgEntry."Entry No." := Doc."Entry No.";
                CustLedgEntry."Customer No." := Doc."Account No.";
                CustLedgEntry."Document No." := Doc."Document No.";
                Cust.Get(Doc."Account No.");
                CustLedgEntry."Customer Posting Group" := Cust."Customer Posting Group";
                CustLedgEntry."Global Dimension 1 Code" := Doc."Global Dimension 1 Code";
                CustLedgEntry."Global Dimension 2 Code" := Doc."Global Dimension 2 Code";
            END;
    ...
    begin
        with Doc2 do begin
            ...
            repeat
                ...
                if NOT CustLedgEntry2.GET("Entry No.") THEN BEGIN
                    CustLedgEntry2.INIT;
                    CustLedgEntry2."Entry No." := "Entry No.";
                    CustLedgEntry2."Customer No." := "Account No.";
                    Cust.Get("Account No.");
                    CustLedgEntry2."Document No." := "Document No.";
                    CustLedgEntry2."Customer Posting Group" := Cust."Customer Posting Group";
                    CustLedgEntry2."Global Dimension 1 Code" := "Global Dimension 1 Code";
                    CustLedgEntry2."Global Dimension 2 Code" := "Global Dimension 2 Code";
                END;
    ...
    /// SetHidePrintDialog.
    /// </summary>
    /// <param name="NewHidePrintDialog">Boolean.</param>
    [Scope('OnPrem')]
