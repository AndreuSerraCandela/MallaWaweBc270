/// <summary>
/// Report Balance Malla (ID 50033).
/// </summary>
report 50028 "Balance Malla"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layout/Balance.rdlc';
    AdditionalSearchTerms = 'Cierre ejercicio,Cierre periodo,close fiscal year';
    ApplicationArea = Basic, Suite;
    Caption = 'Balance Sumas y Saldos Malla';
    ...
    dataset
    {
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            column(IncluyeEntries; IncluyeEntries)
            {
            }
            ...
            begin
                ...
                if GlobalNo < 1 then begin
                    ...
                    if OpenEntries and not CloseEntries then
                        IncluyeEntries := Text1100005;
                    if CloseEntries and not OpenEntries then
                        IncluyeEntries := Text1100006;
                    if CloseEntries and OpenEntries then
                        IncluyeEntries := Text1100007;
                    GlobalNo := GlobalNo + 1;
                end;
            ...
            var
                ...
                FilterEnumAcc: Enum "G/L Account Type";
            begin
                UndefinedValue := -1;
                FromFec := 0D;
                ...
                if GLFilter <> '' then begin
                    ...
                    FilterEnumAcc := GetRangeMin("Account Type");
                    case FilterEnumAcc of
                        FilterEnumAcc::"Begin-Total":
                            GLFilterOption := UndefinedValue;
                        FilterEnumAcc::"End-Total":
                            GLFilterOption := UndefinedValue;
                        FilterEnumAcc::Heading:
                            GLFilterOption := GLFilterOption::Heading;
                        FilterEnumAcc::Posting:
                            GLFilterOption := GLFilterOption::Posting;
                        FilterEnumAcc::"Total":
                            GLFilterOption := UndefinedValue;
                    end;
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Solo cuentas con saldo a la fecha';
                        MultiLine = true;
                        ToolTip = 'Specifies if only the general ledger accounts with a balance are Incluyed in the report.';
                    }
                    field(ShowAmountsInAddCurrency; PrintAmountsInAddCurrency)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Muestra importes en divisa adicional';
                        ToolTip = 'Specifies if amounts in the additional currency are Incluyed.';
                    }
                    field(AcumBalanceAtDate; AcumBalance)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Saldo acum. a fecha';
                        ToolTip = 'Specifies if the report displays an accumulative balance.';
                    }
                    field(IncluyeOpeningEntries; OpenEntries)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Incluye movs. apertura';
                        ToolTip = 'Specifies if opening entries are Incluyed in the report.';
                    }
                    field(IncluyeClosingEntries; CloseEntries)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Incluye Mov. Cierre';
                        ToolTip = 'Specifies if Mov. Cierre are Incluyed in the report.';
                    }
                }
            }
        }
            ...
            area(Processing)
            {
                action("Guardar Filtro")
                {
                    ...
                    begin
                        ...
                        Filtros.Domingo := PrintAllHavingBal;
                        Filtros."Sábado" := PrintAmountsInAddCurrency;
                        Filtros.Viernes := AcumBalance;
                        Filtros.Jueves := OpenEntries;
                        Filtros."Miércoles" := CloseEntries;
                        if evaluate(Filtros.Filtros, "G/L Account".TableName + ': ' + "G/L Account".GetFilters) Then;
                        Filtros."Filtro dimension Zona" := "G/L Account".GetFilter("No.");
                        Filtros."Filtro Familia" := "G/L Account".GetFilter("Date Filter");
                        Filtros."Filtro Tipo Recurso" := "G/L Account".GetFilter("Account Type");
                        Filtros.Insert();
                    end;
                }
                action("Aplicar Filtro")
                {
                    ...
                    begin
                        ...
                        if Page.RunModal(Page::Filtros, Filtros) = Action::LookupOK Then begin
                            // Desde := Filtros.Desde;
                            // Hasta := Filtros.Hasta;
                            PrintAllHavingBal := Filtros.Domingo;
                            PrintAmountsInAddCurrency := Filtros."Sábado";
                            AcumBalance := Filtros.Viernes;
                            OpenEntries := Filtros.Jueves;
                            CloseEntries := Filtros."Miércoles";
                            if Filtros."Filtro dimension Zona" <> '' Then "G/L Account".SetFilter("No.", Filtros."Filtro dimension Zona");
                            if Filtros."Filtro Familia" <> '' Then "G/L Account".SetFilter("Date Filter", Filtros."Filtro Familia");
                            if Filtros."Filtro Tipo Recurso" <> '' Then "G/L Account".SetFilter("Account Type", Filtros."Filtro Tipo Recurso");
                            // Martes := Filtros.Martes;
                            // Lunes := Filtros.Lunes;
                            //"G/L Account".SetFilter("G/L Account".ac);
                        end;
                    end;
                }
            }
    ...
    var
        Text1100002: Label 'La fecha %1 no es a Fecha Cierre';
        Text1100003: Label 'Periodo: ';
        Text1100004: Label 'Todo los importes en %1';
        Text1100005: Label 'Incluye movs. apertura';
        Text1100006: Label 'Incluye Mov. Cierre';
        Text1100007: Label 'Incluye Mov. Cierre/Apertura';
        Text1100008: Label 'El periodo contable no existe';
        ...
        TrialBalanceCaptionLbl: Label 'Balance Sumas y Saldos';
        PageCaptionLbl: Label 'Pag.';
        AccinPeriodCaptionLbl: Label 'Acum. Periodo';
        AccPeriodatDateCaptionLbl: Label 'Acumm. periodo a fecha';
        AccountCaptionLbl: Label 'Cuenta';
        NameCaptionLbl: Label 'Nombre';
        DebitCaptionLbl: Label 'Debe';
        CreditCaptionLbl: Label 'Haber';
        BalanceatDateCaptionLbl: Label 'Saldo a la fecha';
        AcumBalanceatDateCaptionLbl: Label 'Saldo a la fecha Acum.';
