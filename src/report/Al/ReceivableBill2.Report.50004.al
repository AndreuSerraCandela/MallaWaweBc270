/// <summary>
/// Report Receivable Bill2. (ID 50020).
/// </summary>
report 50017 "Receivable Bill2."
{
    // 001 21/09/15 - Impedimos guardar numero pagare si es vista preliminar.
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layout/ReceivableBill2.50004.rdlc';
    ...
    dataset
    {
        dataitem(CustLedgEntry; 25)
        {
            DataItemTableView = SORTING("Entry No.")
                                WHERE("Document Situation" = FILTER("Posted BG/PO" | "BG/PO" | Cartera));
            ...
            column(Dia2; FORMAT(DrawDate, 0, '<Day,2>'))
            {
            }
            ...
            begin
                ...
                CASE "Document Situation" OF
                    "Document Situation"::"BG/PO", "Document Situation"::Cartera:
                        begin
                            // WITH Doc. DO BEGIN
                            Doc.GET(Doc.Type::Payable, CustLedgEntry."Entry No.");
...
#pragma warning restore AL0432
                    ...
                    "Document Situation"::"Posted BG/PO":
                        BEGIN
                            PostedDoc.GET(PostedDoc.Type::Payable, CustLedgEntry."Entry No.");
...
#pragma warning restore AL0432
                ...
                if Customer."Pay-to Vendor No." <> '' THEN
                    if Customer.Get(Customer."Pay-to Vendor No.") Then
                        VendPmtAddress(CustAddr, CustPmtAddress)
                    ELSE BEGIN
                ...
                /*  FindApplnEntriesDtldtLedgEntry;
                  ...
                  rVendenTry2.SETRANGE("Closed by Entry No.",CreateVendLedgEntry."Entry No.");
                ...
                CreateVendLedgEntry.Usuario := NumPag;

                if NOT CurrReport.PREVIEW THEN //+001
                    if rvendledgerentry.GET(CustLedgEntry."Entry No.") THEN BEGIN
                        rvendledgerentry.Usuario := NumPag;
                        rvendledgerentry.MODIFY;
                        Doc."Nº Impreso" := NumPag;
                        Doc."¨Esta Impreso?" := Doc."¨Esta Impreso?"::Si;
                        Doc.MODIFY;
                    END;
                rVendenTry2.RESET;
                rVendenTry2.SETRANGE("Pagaré/Confirming", "Document No.");
                CLEAR(Facturas1);
                CLEAR(Facturas2);
                CLEAR(Facturas3);
                CLEAR(Facturas4);
                ...
                a := 0;
                a := a + 1;
                if rVendenTry2.FINDFIRST THEN
                    REPEAT
                        if rVendenTry2."Tipo documento" = rVendenTry2."Tipo documento"::"Credit Memo" THEN
                            rVendenTry3.SETRANGE("Document Type", rVendenTry3."Document Type"::"Credit Memo")
                        ELSE
                            rVendenTry3.SETRANGE("Document Type", rVendenTry3."Document Type"::Invoice);
                        rVendenTry3.SETRANGE("Document No.", rVendenTry2.Numero);
                        if NOT rVendenTry3.FINDFIRST THEN rVendenTry3.INIT;
                        if rVendenTry2."Nº Externo" = '' THEN rVendenTry2."Nº Externo" := rVendenTry2.Numero;
                        if a = 1 THEN Facturas1 := FORMAT(rVendenTry2."Tipo documento");//+' '+rVendenTry2."Nº Externo";
                        if a = 2 THEN Facturas2 := FORMAT(rVendenTry2."Tipo documento");//+' '+rVendenTry2."Nº Externo";
                        if a = 3 THEN Facturas3 := FORMAT(rVendenTry2."Tipo documento");//+' '+rVendenTry2."Nº Externo";
                        if a = 4 THEN Facturas4 := FORMAT(rVendenTry2."Tipo documento");//+' '+rVendenTry2."Nº Externo";
                        if a = 5 THEN Facturas5 := FORMAT(rVendenTry2."Tipo documento");//+' '+rVendenTry2."Nº Externo";
                        if a = 1 THEN Fechas1 := rVendenTry3."External Document No.";
                        if a = 2 THEN Fechas2 := rVendenTry3."External Document No.";
                        if a = 3 THEN Fechas3 := rVendenTry3."External Document No.";
                        if a = 4 THEN Fechas4 := rVendenTry3."External Document No.";
                        if a = 5 THEN Fechas5 := rVendenTry3."External Document No.";
                        ...
                        rVendenTry3.CALCFIELDS(Amount);
                        if a = 1 THEN Importe1 := -rVendenTry2."Importe a pagar";
                        if a = 2 THEN Importe2 := -rVendenTry2."Importe a pagar";
                        if a = 3 THEN Importe3 := -rVendenTry2."Importe a pagar";
                        if a = 4 THEN Importe4 := -rVendenTry2."Importe a pagar";
                        if a = 5 THEN Importe5 := -rVendenTry2."Importe a pagar";
            ...
            begin
                if NOT CurrReport.PREVIEW THEN BEGIN //+001
                                                     // r270.GET(BillGr."Bank Account No.");
                                                     // r270."Last Remittance Advice No":=NumPag;
                                                     // r270.MODIFY;
                END;
            end;
        }
    ...
    requestpage
    {
        ...
        begin
            ...
            DrawDate := WORKDATE;
            //r270.GET('57200001');
            //NumPag:=r270."Last Remittance Advice No";
        end;
    }
    ...
    var
        ...
        rVendenTry3: Record 25;
        rVendenTry2: Record 7001193;
        NumPag: Code[10];
        r270: Record 270;
        Facturas1: Text[250];
        Facturas2: Text[250];
    ...
    /// <param name="VendPmtAddress">VAR Record "Vendo".</param>
